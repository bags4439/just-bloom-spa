...
const StepIndicator: React.FC<{ current: SaleStep; flowType: FlowType }> = ({
  current,
  flowType,
}) => {
  const steps = flowType === 'sale'
...
interface Step1Props {
  flowType: FlowType;
  onFlowTypeChange: (t: FlowType) => void;
  ...
  return (
    <div className="max-w-lg">
      <div className="mb-5 flex gap-2">
        {(['sale', 'expense'] as FlowType[]).map((t) => (
          <button
            key={t}
            onClick={() => onFlowTypeChange(t)}
            className={cn(
      ...
      {selectedCustomer && (
        <div className="mt-3 rounded-lg bg-primary-pale px-4 py-3 text-sm">
          ...
          <span className="ml-2 text-text-secondary">({selectedCustomer.loyaltyPoints} pts)</span>
          <button
            onClick={() => onSelectCustomer(null)}
...
const paymentSchema = z.object({
  ...
  amountPaid: z.string().refine((v) => !isNaN(parseFloat(v)) && parseFloat(v) >= 0, {
        ...
        >
          ...
          {channel === 'split' && (
            <div className="flex flex-col gap-3 rounded-lg bg-cream p-3">
              <Input
                label="Cash amount (GHS)"
                type="number"
                step="0.01"
                min="0"
                placeholder="0.00"
                {...register('splitCashAmount')}
              />
              <div>
              ...
              <Input
                label="Second amount (GHS)"
                type="number"
                step="0.01"
                min="0"
                placeholder="0.00"
                {...register('splitSecondAmount')}
              />
              <Input
                label="Reference No."
                placeholder="Enter reference number"
                {...register('splitSecondRef')}
              />
            </div>
          )}
...
const ExpenseForm: React.FC<ExpenseFormProps> = ({ onSubmit, onBack, isSubmitting }) => {
  ...
  <div className="max-w-md">
    <Card padding="none" className="overflow-hidden">
      <div className="bg-primary p-8 text-center">
        <div className="mx-auto mb-3 flex h-13 w-13 items-center justify-center rounded-full bg-white/20">
...
const NewTransactionPage: React.FC = () => {
  ...
  const handleFlowTypeChange = useCallback((t: FlowType) => {
    setFlowType(t);
    setSelectedServices([]);
  ...
  const handlePaymentSubmit = useCallback(
    async (data: PaymentFormData): Promise<void> => {
      ...
      try {
        ...
        const timestamp = data.customTimestamp
          ? new Date(data.customTimestamp)
          : new Date();
    ...
    [
      user, sessionId, grossPesewas, selectedCustomer,
      selectedServices, transactionService, triggerRefresh, addToast,
    ],
  );
  ...
  const handleExpenseSubmit = useCallback(
    ...
    [user, sessionId, transactionService, triggerRefresh, addToast, navigate],
  );
  ...
  if (result) {
    return (
      <div className="p-9">
        <SuccessScreen result={result} onDone={handleDone} />
      </div>
    );
  }
  ...
  return (
    <div className="p-9">
      <PageHeader title="New transaction" subtitle="Record a service or expense" />
      <StepIndicator current={step} flowType={flowType} />

      {flowType === 'expense' ? (
        <ExpenseForm
          onSubmit={handleExpenseSubmit}
          onBack={() => { setFlowType('sale'); setStep(1); }}
